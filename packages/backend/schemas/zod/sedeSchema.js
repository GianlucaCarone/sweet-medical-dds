...
export const nombreSedeSchema = z
  .string({
  ...
  .min(
    MIN_NOMBRE_SEDE,
    `El nombre debe tener al menos ${MIN_NOMBRE_SEDE} caracteres`,
  )
  .max(
    MAX_NOMBRE_SEDE,
    `El nombre debe tener como máximo ${MAX_NOMBRE_SEDE} caracteres`,
  )
  .regex(
...
export const direccionSedeSchema = z
  .string({
  ...
  .min(
    MIN_DIRECCION_SEDE,
    `La dirección debe tener al menos ${MIN_DIRECCION_SEDE} caracteres`,
  )
  .max(
    MAX_DIRECCION_SEDE,
    `La dirección debe tener como máximo ${MAX_DIRECCION_SEDE} caracteres`,
  )
  .regex(
    /^[A-Za-zÁÉÍÓÚáéíóúÑñ0-9 .,-]+$/,
